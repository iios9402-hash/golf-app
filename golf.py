...
# 矢板CCの座標。単位をmetric(mm, m/s)に明示的に指定
API_URL = "https://api.open-meteo.com/v1/forecast?latitude=36.8039&longitude=139.9042&daily=precipitation_sum,wind_speed_10m_max&timezone=Asia%2FTokyo&wind_speed_unit=ms&forecast_days=10"
...
st.write("プロオーディオ評論家「百十番」様専用（高精度・10日間解析モデル）")

df = fetch_weather_data()

# 1. 10日間判定（全表示）
st.subheader("🌞 向こう10日間の気象判定")
if not df.empty:
