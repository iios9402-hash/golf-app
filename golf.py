...
# 2週間分の天気・降水・風速、および天気コード(weather_code)を取得
API_URL = "https://api.open-meteo.com/v1/forecast?latitude=36.8039&longitude=139.9042&daily=weather_code,precipitation_sum,wind_speed_10m_max&timezone=Asia%2FTokyo&wind_speed_unit=ms&forecast_days=14"

if 'confirmed_reservation' not in st.session_state:
    st.session_state.confirmed_reservation = st.query_params.get("date", None)

def get_weather_desc(code):
    """天気コードを日本語に変換し、雨が含まれるか判定"""
    # WMO Weather interpretation codes (WW)に基づく簡易変換
...
def fetch_weather_data():
    ...
    try:
        ...
        for i in range(len(daily['time'])):
            ...
            # 11-13日目の特別ルール: 「雨」の文字が入れば不可
            # インデックスiは0から始まるため、10, 11, 12が11, 12, 13日目に相当
            if i in [10, 11, 12]:
                if is_rain:
                    status = "× 不可"
...
st.write("プロオーディオ評論家「百十番」様専用（2週間判定強化モデル）")
...
if not df.empty:
    ...
    st.caption(f"情報源: [tenki.jp 矢板カントリークラブ以前の予報]({TENKI_JP_URL})")
else:
    st.error("データ取得エラー。")
