...
# APIの座標と設定を1つに絞り、タイムアウトを極限まで短縮してフリーズを防止
...
def fetch_weather_fast():
    """最短時間で天気を取得する。失敗時は即座に空データを返す。"""
    try:
        # タイムアウトを5秒に短縮し、無限ループを回避
...
def load_settings_fast():
    """設定読込。失敗しても即座にデフォルトを返す。"""
    if not GITHUB_TOKEN or not REPO_NAME: return {"date": None, "emails": ""}, None
...
# 1. まずUIと設定をロード（天気待たずに表示開始）
settings_data, file_sha = load_settings_fast()
...
# 3. 天気データ取得（ここで少しだけ待つが、タイムアウト設定により無限ループは防ぐ）
df = fetch_weather_fast()
...
if not df.empty:
    ...
    st.warning("現在、気象データを一時的に取得できません。リロードをお試しください。")
...
with col1:
    ...
    if st.session_state.confirmed_reservation:
        try: d_val = datetime.strptime(st.session_state.confirmed_reservation, '%Y-%m-%d')
        except: pass
    ...
    if st.button("設定を完全に保存する"):
        ...
        url = f"https://api.github.com/repos/{REPO_NAME}/contents/{FILE_PATH}"
        headers = {"Authorization": f"token {GITHUB_TOKEN}", "Accept": "application/vnd.github.v3+json"}
        content_json = json.dumps({"date": date_str, "emails": new_emails_str}, ensure_ascii=False)
        data = {"message": "Sync", "content": base64.b64encode(content_json.encode('utf-8')).decode('utf-8')}
        if file_sha: data["sha"] = file_sha
        ...
        try:
            ...
            if res.status_code in [200, 201]:
                ...
                st.success("同期完了。")
                st.rerun()
            else: st.error("GitHub連携エラー。Rebootしてください。")
...
with col2:
    ...
    if st.session_state.confirmed_reservation and not df.empty:
        ...
        if not res_info.empty:
            ...
            if curr["判定"] == "× 不可": st.error(f"⚠️ 警告: {curr['曜日付き']} は【{curr['理由']}】です。")
            else: st.success(f"✅ 良好: {curr['曜日付き']} は条件をクリアしています。")
    else: st.info("信号待ち：予報または予約日が未確定です。")
...
if st.button("📩 登録全アドレスへテストメール送信"):
    ...
    for email in all_recipients:
        requests.post("https://ntfy.sh/yaita_golf_110", data=body.encode('utf-8'),
                      headers={"Title": f"【矢板CC】判定({target})".encode('utf-8'), "Email": email}, timeout=5)
    st.success("テスト送信完了。")
