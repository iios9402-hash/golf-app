...
# --- 永続的な記憶保持 (Browser Local Storage) ---
# 予約日と追加メールアドレスをブラウザ自体に記憶させます
...
if 'additional_emails' not in st.session_state:
    ...
    st.session_state.additional_emails = saved_emails.split(",") if saved_emails else []
...
with col1:
    ...
    # 予約日の入力
    try: d_val = datetime.strptime(st.session_state.confirmed_reservation, '%Y-%m-%d') if st.session_state.confirmed_reservation else datetime.now()
    ...
    new_emails_str = st.text_area("追加通知先メールアドレス（カンマ区切り）", value=current_emails, placeholder="example@test.com, sample@test.jp")
    ...
    if st.button("設定をブラウザに保存"):
        ...
        # URLパラメータに書き込んで保存（ブラウザを閉じても復元可能にする）
        ...
        st.success("設定を保存しました。次回起動時も維持されます。")
        st.rerun()
